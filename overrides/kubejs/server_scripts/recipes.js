...
function unwantedRecipes(event) {
	event.remove({ output: "ae2:certus_crystal_seed" });
	event.remove({ output: "ae2:fluix_crystal_seed" });
	event.remove({ output: "beyond_earth:steel_engine" });
	event.remove({ output: "beyond_earth:desh_engine" });
	event.remove({ output: "beyond_earth:ostrum_engine" });
	...
	event.remove({ output: 'beyond_earth:rocket_t2' });
	event.remove({ output: 'beyond_earth:rocket_t3' });
	event.remove({ output: 'beyond_earth:rocket_t4' });
	event.remove({ id: "thermal:machines/smelter/smelter_silver_plate_to_ingot" });
	event.remove({ type: TE("smelter"), output: TE("enderium_ingot") });
	event.remove({ output: '#forge:coins' })
	event.remove({ output: '#forge:ores/silver' })
	...
	event.remove({ input: '#forge:raw_materials/silver' })
	event.remove({ input: 'darkerdepths:limestone_silver_ore' })
	event.remove({ input: 'darkerdepths:aridrock_silver_ore' })
	...
	event.remove({ mod: 'structures_compass' })
...
function tweaks(event) {
	...
	event.smithing("beyond_earth:rocket_launch_pad", AP("heavy_stone_bricks"), KJ("matter_plastics"))
	event.smithing("beyond_earth:calorite_tank", "beyond_earth:steel_block", AE2("sky_stone_tank"))
	...
	event.replaceInput({ id: "beyond_earth:calorite_engine" }, "beyond_earth:desh_plate", KJ('matter_plastics'))
	event.replaceInput({ id: "beyond_earth:calorite_engine" }, "beyond_earth:calorite_ingot", CR('blaze_burner'))
	...
	let tweak_casing = (name, mats, mod) => {
		...
		event.recipes.createDeploying(mod(name + "_casing"), mats)
	...
	tweak_casing('refined_radiance', ['#forge:stripped_logs', CR('refined_radiance')], CR)
	tweak_casing('shadow_steel', ['#forge:stripped_logs', CR('shadow_steel')], CR)
	tweak_casing('zinc', ['minecraft:stone', KJ('zinc_sheet')], KJ)
	tweak_casing('invar', ['minecraft:stone', TE('invar_ingot')], KJ)
	tweak_casing('enderium', ['minecraft:obsidian', MC('ender_pearl')], KJ)
	tweak_casing('fluix', ['minecraft:blackstone', TE('lead_plate')], KJ)
...
*/
	...
	event.replaceInput({ id: "beyond_earth:solar_panel" }, MC('lapis_lazuli'), AE2('controller'))
	event.replaceInput({ id: "beyond_earth:coal_generator" }, MC('furnace'), AE2('controller'))
	event.replaceInput({ id: "beyond_earth:compressor" }, MC('furnace'), AE2('controller'))
...
function rocketScience(event) {
	...
	event.remove({ output: "beyond_earth:fuel" });
	...
	event.recipes.createMechanicalCrafting("beyond_earth:fuel_refinery", [
		'AAA',
		'GSG',
		'AMA'
	], {
...
function unify(event) {
	...
	event.replaceInput({}, '#forge:dusts/quartz', AE2('certus_quartz_dust'))
	event.replaceOutput({}, TE("quartz_dust"), AE2("certus_quartz_dust"))
...
function electronTube(event) {
	...
	event.recipes.createMixing(['create:polished_rose_quartz'], [[/*AE2('purified_nether_quartz_crystal'),*/ AE2('certus_quartz_crystal')], Fluid.of(TE("redstone"), 250)])
...
function brassMachine(event) {
	...
	event.recipes.createSequencedAssembly([
		...
		event.recipes.createDeploying(t, [t, CR('electron_tube')]),
		event.recipes.createDeploying(t, [t, F('#fillet_knife')])
	]).transitionalItem(t)
		.loops(1)
...
function enderMachine(event) {
	...
	event.recipes.thermal.smelter(TE("enderium_ingot"), [F("#ingots/silver"), MC("bubble_coral"), MC("ender_pearl")]).energy(10000)
	event.recipes.thermal.smelter(TE("enderium_ingot"), [F("#ingots/silver"), MC("bubble_coral"), AE2("ender_dust", 4)]).energy(10000)
...
function alchemy(event) {
	...
	alchemy_mix("blue", "herbal", "cobblestone", "basalt")
	alchemy_mix("magenta", "herbal", "andesite", "cobblestone")

	alchemy_smelt("nether", "volatile", "red", "cobblestone")
	...
	alchemy_smelt("cobblestone", "igneous", "cinnabar", "zinc", 20)
...
function trading(event) {
	...
	global.professions.forEach(element => {
		if (global.professions[element])
			global.professions[element].forEach(sale => {
				trade(KJ('profession_card_' + element), sale.in, sale.out)
			})
	});
}
